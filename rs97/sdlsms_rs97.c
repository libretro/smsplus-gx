...
void Menu()
{
    ...
    SDL_Surface* sdlscreen = SDL_CreateRGBSurface(SDL_SWSURFACE, 320, 240, 16, 0, 0, 0, 0);
    ...
    while (((currentselection != 1) && (currentselection != 6)) || (!pressed))
    {
        ...
        dstRect.x = sdlscreen->w-5-miniscreenwidth;
        dstRect.y = 30;
        SDL_BlitSurface(miniscreen,NULL,sdlscreen,&dstRect);

        gfx_font_print_center(sdlscreen,5,bigfontwhite,"SMS_SDL 0.9.4aR7.1");

        if (currentselection == 1)
            gfx_font_print(sdlscreen,5,25,bigfontred,"Continue");
        else
            gfx_font_print(sdlscreen,5,25,bigfontwhite,"Continue");

        sprintf(text,"Save State %d",sdl_controls.state_slot);

        if (currentselection == 2)
            gfx_font_print(sdlscreen,5,65,bigfontred,text);
        else
            gfx_font_print(sdlscreen,5,65,bigfontwhite,text);

        sprintf(text,"Load State %d",sdl_controls.state_slot);

        if (currentselection == 3)
            gfx_font_print(sdlscreen,5,85,bigfontred,text);
        else
            gfx_font_print(sdlscreen,5,85,bigfontwhite,text);

        if (currentselection == 4)
        {
            if (fullscreen == 1)
                gfx_font_print(sdlscreen,5,105,bigfontred,"Stretched");
            else
				gfx_font_print(sdlscreen,5,105,bigfontred,"Native");
        }
        else
        {
            if (fullscreen == 1)
                gfx_font_print(sdlscreen,5,105,bigfontwhite,"Stretched");
            else
				gfx_font_print(sdlscreen,5,105,bigfontwhite,"Native");
        }

        sprintf(text,"%s",showfps ? "Show fps on" : "Show fps off");

        if (currentselection == 5)
            gfx_font_print(sdlscreen,5,125,bigfontred,text);
        else
            gfx_font_print(sdlscreen,5,125,bigfontwhite,text);


        if (currentselection == 6)
            gfx_font_print(sdlscreen,5,145,bigfontred,"Quit");
        else
            gfx_font_print(sdlscreen,5,145,bigfontwhite,"Quit");

        gfx_font_print_center(sdlscreen,sdlscreen->h-40-gfx_font_height(font),font,"SMS_SDL for the RS-97");
        gfx_font_print_center(sdlscreen,sdlscreen->h-30-gfx_font_height(font),font,"RS-97 port by gameblabla");
        gfx_font_print_center(sdlscreen,sdlscreen->h-20-gfx_font_height(font),font,"Initial port: exmortis@yandex.ru,joyrider");
        gfx_font_print_center(sdlscreen,sdlscreen->h-10-gfx_font_height(font),font,"Scaler code: Alekmaul, Harteex: TGA,bin2h");
		...
		SDL_SoftStretch(sdlscreen, NULL, sdl_video.surf_screen, NULL );
...
static void sdlsms_video_finish_update()
{
	...
	switch(fullscreen) 
	{
        ...
        case 2: // aspect ratio
		if(IS_GG) 
			upscale_160x144_to_320x480((uint32_t*)sdl_video.surf_screen->pixels, (uint32_t*)sdl_video.surf_bitmap->pixels+((24*256)+48)/2);
		else 
			/* (256*2) removes the 2 black pixels on the top, so effectively it's 256x190, not 256x192 */
			upscale_256x192_to_320x480((uint32_t*)sdl_video.surf_screen->pixels, (uint32_t*)sdl_video.surf_bitmap->pixels+(256*2));
		break;
        default: // native res
		if(IS_GG) 
			bitmap_scale(48,24,160,144,160,288,256,sdl_video.surf_screen->w-160,(uint16_t*)sdl_video.surf_bitmap->pixels,(uint16_t*)sdl_video.surf_screen->pixels+(sdl_video.surf_screen->w-160)/2+(sdl_video.surf_screen->h-288)/2*sdl_video.surf_screen->w);
		else 
			bitmap_scale(0,0,256,192,256,384,256,sdl_video.surf_screen->w-256,(uint16_t*)sdl_video.surf_bitmap->pixels,(uint16_t*)sdl_video.surf_screen->pixels+(sdl_video.surf_screen->w-256)/2+(sdl_video.surf_screen->h-384)/2*sdl_video.surf_screen->w);
		break;
	}
