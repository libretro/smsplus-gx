...
static uint8_t z80_exec = 0;               /* 1= in exec loop, 0= out of */
static int32_t z80_requested_cycles = 0;   /* requested cycles to execute this timeslice */
...
/****************************************************************************/
INLINE void BURNODD(int32_t cycles, int32_t opcodes, int32_t cyclesum)
{
  if( cycles > 0 )
  {
 ...
 ***************************************************************/
INLINE void RM16( uint32_t addr, PAIR *r )
{
  r->b.l = RM(addr);
 ...
 ***************************************************************/
INLINE void WM16( uint32_t addr, PAIR *r )
{
  WM(addr,r->b.l);
...
#define INI {                          \
  ...
  HL++;                            \
  F = SZ[B];                          \
  t = (uint32_t)((C + 1) & 0xff) + (uint32_t)io;        \
  if( io & SF ) F |= NF;                    \
  if( t & 0x100 ) F |= HF | CF;                \
...
OP(op,d3) { uint32_t n = ARG() | (A << 8); OUT( n, A ); WZ_L = ((n & 0xff) + 1) & 0xff;  WZ_H = A; } /* OUT  (n),A       */
...
OP(op,db) { uint32_t n = ARG() | (A << 8); A = IN( n ); WZ = n + 1;                                    } /* IN   A,(n)       */
