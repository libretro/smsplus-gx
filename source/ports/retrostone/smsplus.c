...
static void video_update(void)
{
	...
	switch(option.fullscreen)
	{
		...
		if(sms.console == CONSOLE_GG) 
			...
			256,vdp.height,
			256*2,vdp.height*2,
			256, sdl_screen->w-256*2,
			(uint16_t* restrict)sms_bitmap->pixels,(uint16_t* restrict)sdl_screen->pixels+(sdl_screen->w-(256*2))/2+(sdl_screen->h-(vdp.height*2))/2*sdl_screen->w);
		break;
		// Standard fullscreen
		case 1:
		if(sms.console == CONSOLE_GG) 
...
void Menu()
{
    ...
    while (((currentselection != 1) && (currentselection != 6)) || (!pressed))
    {
        ...
        if (currentselection == 4)
        {
			...
			{
				case 0:
					print_string("Scaling : Native", TextBlue, 0, 5, 105, backbuffer->pixels);
				break;
				case 1:
					print_string("Scaling : Stretched", TextBlue, 0, 5, 105, backbuffer->pixels);
				break;
				case 2:
					print_string("Scaling : EPX/Scale2x", TextBlue, 0, 5, 105, backbuffer->pixels);
				break;
			}
        }
        else
		...
		if (currentselection == 5) print_string(text, TextBlue, 0, 5, 125, backbuffer->pixels);
		else print_string(text, TextWhite, 0, 5, 125, backbuffer->pixels);
		
		if (currentselection == 6) print_string("Quit", TextBlue, 0, 5, 145, backbuffer->pixels);
		else print_string("Quit", TextWhite, 0, 5, 145, backbuffer->pixels);
		
		print_string("Based on SMS Plus by Charles Mcdonald", TextWhite, 0, 5, 175, backbuffer->pixels);
		print_string("Fork of SMS Plus GX by gameblabla", TextWhite, 0, 5, 190, backbuffer->pixels);
		print_string("Scaler : Alekmaul", TextWhite, 0, 5, 205, backbuffer->pixels);
		print_string("Text drawing : n2DLib", TextWhite, 0, 5, 220, backbuffer->pixels);
