...
static uint16_t* sdl_screen;
uint16_t *sms_bitmap;

static char home_path[256];

#ifdef SCALE2X_UPSCALER
static uint16_t* scale2x_buf;
static uint32_t dst_x, dst_w, dst_h;
#endif

extern uint16_t *font;
extern uint16_t *bigfontred;
extern uint16_t *bigfontwhite;
...
void Menu()
{
    ...
    uint16_t* miniscreen = SDL_CreateRGBSurface(SDL_SWSURFACE, miniscreenwidth, miniscreenheight, 16, sdl_screen->format->Rmask, sdl_screen->format->Gmask, sdl_screen->format->Bmask, sdl_screen->format->Amask);
