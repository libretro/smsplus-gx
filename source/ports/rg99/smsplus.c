...
{
	uint_fast16_t height, width, pixels_shifting_remove;
	uint32_t i;
	...
	SDL_BlitSurface(sms_bitmap, &rct, sdl_screen, NULL);
	SDL_Flip(sdl_screen);
}
...
{
	if (h == 0) h = 192;
	sdl_screen = SDL_SetVideoMode(w, h, 8, SDL_HWSURFACE | SDL_TRIPLEBUF);
	return sdl_screen ? 1 : 0;
}
